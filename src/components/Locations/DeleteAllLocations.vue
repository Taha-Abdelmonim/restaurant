  ...
  async mounted() {
    ...
    if (!user) {
      ...
      let resultLoc = await axios.get(
        `https://fake-jsonapi.herokuapp.com/locations?userId=${this.userId}`
      );
      ...
      let resultCatLoc = await axios.get(
        `https://fake-jsonapi.herokuapp.com/categories?userId=${this.userId}`
      );
      ...
      let resultItemsLoc = await axios.get(
        `https://fake-jsonapi.herokuapp.com/items?userId=${this.userId}`
      );
    ...
    async deleteAllLocation() {
      ...
      for (var c = 0; c < this.allItemsIdIs.length; c++) {
        let result2 = await axios.delete(
          `https://fake-jsonapi.herokuapp.com/items/${this.allItemsIdIs[c]}`
        );
        console.log(result2);
      ...
      for (var v = 0; v < this.allCatsIdIs.length; v++) {
        let result3 = await axios.delete(
          `https://fake-jsonapi.herokuapp.com/categories/${this.allCatsIdIs[v]}`
        );
        console.log(result3);
      ...
      for (var i = 0; i < this.allLocationId.length; i++) {
        let result = await axios.delete(
          `https://fake-jsonapi.herokuapp.com/locations/${this.allLocationId[i]}`
        );
        console.log(result);
