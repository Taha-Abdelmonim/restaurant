    ...
    <form @click.prevent>
      <div class="row g-3 align-items-center">
        <div class="col-auto mx-auto d-block">
          ...
          <p class="text-danger">When Deleting all categories, thier related menu items will be deleted as well...</p>
          <hr />
        </div>
      </div>
      <div class="row g-3 align-items-center">
        <div class="col-auto mx-auto d-block">
          ...
          &nbsp;&nbsp;&nbsp;
          <button class="btn btn-danger" @click="deleteAllCategories()">Delete All Categories Now</button>
        </div>
      </div>
      <br />
      <div class="row g-3 align-items-center">
...
export default {
  ...
  async mounted() {
    ...
    if (!user) {
      ...
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/items?locId=${this.locationId}`);
      let resultLen = result.data.length;
      for (var i = 0; i < resultLen; i++) {
        this.allItemsIdIs.push(result.data[i].id);
      }
      let resultForCategories = await axios.get(`https://fake-jsonapi.herokuapp.com/categories?locationId=${this.locationId}`);
  ...
  methods: {
    ...
    async canCurrentUserAccessThisLocation() {
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/locations?id=${this.deleteLoction}&userId=${this.userId}`);
    ...
    async deleteAllCategories() {
      ...
      for (var v = 0; v < this.allCatsIdIs.length; v++) {
        let result = await axios.delete(`https://fake-jsonapi.herokuapp.com/categories/${this.allCatsIdIs[v]}`);
        if (result.status == 200) {
      ...
      for (var y = 0; y < this.allItemsIdIs.length; y++) {
        let result = await axios.delete(`https://fake-jsonapi.herokuapp.com/items/${this.allItemsIdIs[y]}`);
        if (result.status == 200) {
