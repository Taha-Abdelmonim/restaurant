    ...
    <form @click.prevent>
      <div class="row g-3 align-items-center">
        <div class="col-auto mx-auto d-block">
          ...
          <p class="text-danger">When Deleting this category, it will delete all related menu items as well...</p>
          <hr />
        </div>
      </div>
      <div class="row g-3 align-items-center">
...
export default {
  ...
  async mounted() {
    ...
    if (!user) {
      ...
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/items?catId=${this.catId}`);
  ...
  methods: {
    ...
    async deleteCategory() {
      let result = await axios.delete(`https://fake-jsonapi.herokuapp.com/categories/${this.catId}`);
      let allResults = [];
      for (var i = 0; i < this.allItemsIdIs.length; i++) {
        let result = await axios.delete(`https://fake-jsonapi.herokuapp.com/items/${this.allItemsIdIs[i]}`);
        if (result.status == 200) {
