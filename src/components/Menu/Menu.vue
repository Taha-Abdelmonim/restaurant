<template>
  ...
  <div class="container">
    ...
    <div class="clearfix"></div>
    <router-link v-if="listOfUserItems.length > 0" :to="{ name: 'DeleteAllItems', params: { locationId: locationId } }">
    ...
    <div class="each-category">
      <div class="" v-for="(cat, i) in listOfUserCategories" :key="i">
        <div class="row">
          ...
          >
            <div v-if="cat.id == item.catId" :class="{ 'each-item': cat.id == item.catId }">
              <h4 class="item-name">{{ item.name }}</h4>
              <p class="item-description text-muted">
                {{ item.description }}
              </p>
              <div>
                <div class="available-items float-start text-muted">Available Qty: {{ numberWithCommas(item.qty) }}</div>
                <div class="item-price float-end text-muted">Price: {{ numberWithCommas(item.price) }}</div>
              </div>
              <div class="clearfix"></div>
              <div class="p-1 mt-2">
...
export default {
  ...
  computed: {
    ...mapState(["isUserLoggedIn", "loggedInUserId", "numOfCategories", "listOfCategories", "listOfLocations"]),
  },
  async mounted() {
  ...
  methods: {
    ...
    async getCurrentUserCategories(userId, locId) {
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/categories?userId=${userId}&locationId=${locId}`);
    ...
    async getCurrentUserItems(userId, locId) {
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/items?userId=${userId}&locId=${locId}`);
      if (result.status == 200) {
