  ...
  <div class="container">
    ...
    <form @click.prevent>
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <div class="form-floating mb-3" :class="{ 'form-group-error': v$.name.$error }">
            <input type="text" class="w250 form-control" id="floatRestCatIs" placeholder="Category Name Is" v-model.trim="name" />
      ...
      <div class="row g-3 align-items-center mb-3">
        <div class="col-auto d-block mx-auto">
          <button type="button" @click="updateCategory()" class="w250 btn btn-warning">Update Now</button>
        </div>
      </div>
      <div class="row g-3 align-items-center mb-3">
...
export default {
  ...
  methods: {
    ...
    async getCategoryName(userId, locId, catId) {
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/categories?userId=${userId}&locationId=${locId}&id=${catId}`);
      if (result.status == 200) {
    ...
    async updateCategory() {
      ...
      if (!this.v$.$error) {
        if (filterCategoryName.length > 0) {
          ...
          let result = await axios.put(`https://fake-jsonapi.herokuapp.com/categories/${this.catId}`, {
            name: this.name,
            userId: this.userId,
