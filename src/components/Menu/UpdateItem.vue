<template>
  ...
  <div class="container">
    ...
    <form @click.prevent>
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <div class="form-floating mb-3" :class="{ 'form-group-error': v$.itemName.$error }">
            <input type="text" class="w250 form-control" id="floatItemName" placeholder="Enter Item Name" v-model.trim="itemName" />
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <div class="form-floating mb-3" :class="{ 'form-group-error': v$.itemPrice.$error }">
            <input type="Number" class="w250 form-control" id="floatItemPrice" placeholder="Enter Item Price" v-model.trim="itemPrice" />
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <div class="form-floating mb-3" :class="{ 'form-group-error': v$.itemsQty.$error }">
            <input type="number" class="w250 form-control" id="floatItemQty" placeholder="Enter Item Quantities" v-model.trim="itemsQty" />
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <button type="button" @click="UpdateItem()" class="w250 btn btn-warning">Update Now</button>
        </div>
      </div>
      <br />
      <div class="row g-3 align-items-center">
...
export default {
  ...
  computed: {
    ...mapState(["isUserLoggedIn", "loggedInUserId", "numOfCategories", "listOfCategories", "listOfLocations", "listOfItems"]),
  },
  validations() {
    return {
  ...
  methods: {
    ...mapMutations(["isLoggedInUser", "displayAllCategories", "canUserAccessThisLocation", "canUserAccessThisItem"]),
    ...mapActions(["redirectTo"]),
    async getItemInfo(userId, locId, itemId) {
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/items?userId=${userId}&locId=${locId}&id=${itemId}`);
    ...
    async getLocationInfo(userId, locId) {
      //https://fake-jsonapi.herokuapp.com/locations?userId=1
      let result = await axios.get(`https://fake-jsonapi.herokuapp.com/locations?userId=${userId}&id=${locId}`);
      let locDetails = [];
      if (result.status == 200) {
    ...
    async UpdateItem() {
      ...
      if (!this.v$.$error) {
        ...
        let results = await axios.put(`https://fake-jsonapi.herokuapp.com/items/${this.itemId}`, {
          name: this.itemName,
