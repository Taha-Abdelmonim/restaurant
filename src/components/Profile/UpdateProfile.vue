<template>
  <div class="container">
    ...
    <form @click.prevent>
      ...
      <div class="row g-3 align-items-center">
        <div class="col-auto d-block mx-auto">
          <button type="submit" @click="updateProfileNow()" class="w250 btn btn-warning">Update Now</button>
        </div>
      </div>
  ...
  methods: {
    ...
    async updateProfileNow() {
      ...
      if (!this.v$.$error) {
        ...
        //put
        let result = await axios.put(`https://fake-jsonapi.herokuapp.com/users/${this.userId}`, {
          name: this.name,
          email: this.email,
          pass: this.pass,
        });
