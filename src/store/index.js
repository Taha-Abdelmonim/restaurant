...
const mutations = {
  ...
  async displayAllCategories(state, payload) {
    let result = await axios.get(`https://fake-jsonapi.herokuapp.com/categories?userId=${payload.userIdIs}&locationId=${payload.locationIdIs}`);
  ...
  async canUserAccessThisLocation(state, payload) {
    let result = await axios.get(`https://fake-jsonapi.herokuapp.com/locations?userId=${payload.userIdIs}&id=${payload.locationIdIs}`);
    if (result.status == 200) {
  ...
  async canUserAccessThisCategory(state, payload) {
    let result = await axios.get(
      `https://fake-jsonapi.herokuapp.com/categories?userId=${payload.userIdIs}&locationId=${payload.locationIdIs}&id=${payload.catIdIs}`
    );
    if (result.status == 200) {
      state.listOfCategories = result.data;
  ...
  async canUserAccessThisItem(state, payload) {
    let result = await axios.get(
      `https://fake-jsonapi.herokuapp.com/items?userId=${payload.userIdIs}&locId=${payload.locationIdIs}&id=${payload.itemIdIs}`
    );
    if (result.status == 200) {
